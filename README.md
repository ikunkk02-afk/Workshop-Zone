...
- JEI、EMI、REI 均为可选兼容，原版配方书普通和 Shift 点击是核心验收路径。
...
9. 在合成栏填充后，从原版输出槽取出成品。

> 当前有效扫描范围以服务端实际逻辑为准（水平 8 格、垂直 4 格）。目标需要处于已加载区块，玩家必须处于允许访问距离。模组不会绕过原版锁和保护回调。

## 配方查看器兼容

- 自动检测 JEI、EMI、REI 的存在。
- AUTO 模式下检测到配方查看器后优先将侧栏放在上方以避开右侧物品列表。
- 玩家可自行在位置菜单中改为左、右、上、下或自定义位置。
- JEI 的公开排除区域已接入。
- 原版配方书是仓库辅助合成的主要支持路径。JEI、EMI、REI 各自独立的配方传输按钮不保证一定触发仓库补齐。
