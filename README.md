...
## 仓库材料补齐合成（Phase 1 / Step 12）

本阶段支持原版工作台 3×3 的标准有序与无序 `CraftingRecipe`。玩家通过原版配方书点击配方时，服务端先按原版库存判断：背包材料足够时完全保留原版流程，不访问工坊容器；背包不足但当前有效工坊的可访问容器能够补齐时，才显示确认面板。面板确认前不移动、不锁定、不预留任何物品，取消也不会改变背包、仓库或合成栏。

确认后服务端从自己的 `RecipeManager` 重新解析配方，重新验证工作台、空的 3×3 合成栏、会话、revision、sync ID、维度、区块、距离、锁和保护回调，再按“玩家主背包与快捷栏优先、仓库只补缺失部分”构建一次合成计划。Ingredient 使用确定性小规模分配器处理 Tag 和重叠候选（先满足候选更少的 Ingredient，并以最少仓库用量为目标），实际来源使用 `ItemVariant`，因此会保留 Data Component。

提取玩家槽、逻辑仓库槽并填入工作台输入槽使用同一个 Fabric Transfer API 外层 Transaction；任何来源数量、权限或目标插入异常都会回滚。成功后只填充真实 3×3 合成栏，不直接生成成品，原版输出槽继续负责结果、统计、解锁和容器剩余物。当前只支持一次合成：不支持 Shift 批量、递归子配方、自动把原木转木板、自动熔炼、其他工作设备或直接把成品放入背包。合成栏在请求时必须为空。

JEI 仍是可选依赖。确认面板复用现有侧边栏 Placement 和 JEI 排除区域；JEI 的公开 Transfer API 未被反射或 internal 包强行接入。若配方查看器没有自然调用原版 `CraftRequestC2SPacket` 流程，本阶段不承诺其独立传输按钮会触发仓库补齐，原版配方书流程是核心验收路径。
